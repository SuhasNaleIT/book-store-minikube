...
# ─────────────────────────────────────────────────────────────
# SEED DATA
# Upserts on every startup — inserts new records or updates
# existing ones by isbn, so redeployments reflect code changes.
# ─────────────────────────────────────────────────────────────
def _seed_books():
    ...
    for book_data in books:
        stmt = pg_insert(Book.__table__).values(**book_data)
        stmt = stmt.on_conflict_do_update(
            index_elements=["isbn"],        # isbn is unique per book
            set_={
